...
(function() {
    ...
    function createUI() {
        ...
        micBtn.innerHTML = '🎤';
        micBtn.onclick = () => { isMicMuted = !isMicMuted; updateBtn(); toggleMicState(); };

        const headBtn = document.createElement('button');
        headBtn.className = 'an-btn';
        headBtn.innerHTML = '🎧';
        headBtn.onclick = () => { isHeadphonesMuted = !isHeadphonesMuted; updateBtn(); toggleHeadState(); };
        ...
        addToggle('Самопрослушивание', 'enableLoopback', (val) => {
            ...
            if (globalStream) AudioEngine.processMicrophone(globalMicStreamOrig).then(s => globalStream = s); 
            ...
            if(globalMicStreamOrig) AudioEngine.processMicrophone(globalMicStreamOrig);
        ...
        lbSlider.type = 'range'; lbSlider.className = 'an-range';
        lbSlider.min = 0.1; lbSlider.max = 3.0; lbSlider.step = 0.1; lbSlider.value = settings.gainValue;
        lbSlider.oninput = (e) => { settings.gainValue = parseFloat(e.target.value); saveSettings(); AudioEngine.updateParams(); };
        ...
        pSlider.type = 'range'; pSlider.className = 'an-range';
        pSlider.min = 0; pSlider.max = 0.4; pSlider.step = 0.01; pSlider.value = settings.pitchLevel;
        pSlider.oninput = (e) => { settings.pitchLevel = parseFloat(e.target.value); saveSettings(); AudioEngine.updateParams(); };
        pDiv.append(pSlider);
        ui.append(pDiv);
        ...
        addToggle('Голосовое управление', 'voiceControl', (val) => {
            if (val) { if(!recognition) initSpeech(); recognition.start(); }
    ...
    function toggleHeadState() {
        ...
        if(isHeadphonesMuted && !isMicMuted) { isMicMuted=true; toggleMicState(); } // Auto mute mic
    }
    ...
    const observer = new MutationObserver(() => {
        ...
        obsTimer = setTimeout(() => {
            ...
            if (isAutoModeEnabled) {
                const btn = document.querySelector('button.callScreen__findBtn, button.go-scan-button, .scan-button');
    ...
    function initSpeech() {
        ...
        recognition.onresult = (e) => {
            ...
            if (CONFIG.voiceCommands.start.some(w=>t.includes(w))) { isAutoModeEnabled=true; }
        };
